...
@section('title', 'نظرات مقالات')
@section('admin_content')
    <div class="row">
        <p class="title p-3">نظرات مقالات</p>
        <form action="{{ route('admin.articles.comments') }}">
            <div class="row mb-4">
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <select name="product_id" class="form-select">
                        ...
                        @foreach ($articles as $article)
                            <option value="{{ $article->id }}" @selected(Request::get('article_id') == $article->id)>{{ $article->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <tbody class="table-group-divider">
                    @foreach ($comments as $key => $comment)
                        <tr>
                            ...
                            @php
                                $article = $articles->firstWhere('id', $comment->article_id);
                            @endphp
                            <td>{{ $article->id }}</td>
                            <td>{{ $article->title }}</td>
                            <td>{{ $comment->like }}</td>
                            <td>{{ $comment->text }}</td>
                            <td><a href="{{ route('admin.articles.show_comment', ['id' => $comment->id]) }}"
                            ...
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer justify-content-start">
                                        <form
                                            action="{{ route('admin.articles.confirm_comment', ['id' => $comment->id]) }}"
                                            method="POST">
                                            @csrf
