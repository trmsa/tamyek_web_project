...
@section('admin_content')
    <p class="title mb-4 p-3">ویرایش اطلاعات {{ $user->name }}</p>
    <form action="{{ route('admin.users.update', ['id' => $user->id]) }}" method="post">
        ...
        <div class="row shadow rounded-4 p-3 justify-content-between">
            ...
            <div class="col-md-5 mb-3">
                ...
                <input type="number" name="mobile" value="{{ $user->mobile }}" class="form-control hidden-arrow" required>
            </div>
            <div class="col-md-5 mb-3">
                <label class="form-label">کدملی (اختیاری)</label>
                <input type="number" name="national_code" value="{{ $user->national_code }}"
                    class="form-control hidden-arrow" placeholder="کدملی">
            </div>
            <div class="col-md-5 mb-3">
                ...
                <div class="row">
                    <div class="col-4">
                        <select name="birth_date_d" class="form-select">
                            <option value=""></option>
                            @for ($i = 1; $i < 32; $i++)
                                <option value="{{ $i }}" @selected($user->birth_date && $user->birth_date['d'] == $i)>{{ $i }}
                                </option>
                            @endfor
                        </select>
                    </div>
                    <div class="col-4">
                        <select name="birth_date_m" class="form-select">
                            <option value="" @selected($user->birth_date && $user->birth_date['m'] == '')></option>
                    ...
                    <div class="col-4">
                        <select name="birth_date_y" class="form-select">
                            ...
                            @for ($i = Helper::fa_date('%y'); $i > 1300; $i--)
                                <option value="{{ $i }}" @selected($user->birth_date && $user->birth_date['y'] == $i)>{{ $i }}
                                </option>
                            @endfor
                        </select>
                    </div>
