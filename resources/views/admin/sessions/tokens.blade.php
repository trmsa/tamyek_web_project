...
@section('title', 'توکن های اپ')
@section('admin_content')
    <div class="row">
        <p class="title p-3">توکن های اپ</p>
        <div class="my-3 text-start">
            <form action="{{ route('admin.clear_tokens') }}" method="POST">
                @csrf
                @method('delete')
                <button class="btn btn-sm btn-danger">پاکسازی توکن‌های اپ</button>
            </form>
        </div>
        <form action="{{ route('admin.tokens') }}">
            <div class="row">
                <div class="col-md-5">
            ...
            <table class="table table-hover table-sm text-center">
                ...
                <tbody class="table-group-divider">
                    ...
                    @foreach ($tokens as $key => $token)
                        @php
                            $user = $users->find($token->tokenable_id);
                        @endphp
                        <tr>
                            <th scope="row">{{ $key + 1 }}</th>
                            <th>{{ $token->tokenable_id ?? '---' }}</th>
                            <th>{{ $user->name ?? '---' }}</th>
                            <th>{{ $user->mobile ?? '---' }}</th>
                            <th>{{ $token->last_used_at ? Helper::fa_date('%y/%m/%d, %H:%s', strtotime($token->last_used_at)) : '---' }}
                            </th>
                            <th><button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal"
                                    data-bs-target="#modalToken{{ $key }}">حذف</button></th>
                            <!-- Modal -->
                            <div class="modal fade" id="modalToken{{ $key }}" tabindex="-1"
                                ...
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer justify-content-start">
                                            <form action="{{ route('admin.delete_token') }}" method="POST">
                                                <input type="hidden" name="user_id" value="{{ $token->tokenable_id }}">
                                                <input type="hidden" name="last_used_at"
        ...
        @if ($tokens instanceof \Illuminate\Pagination\LengthAwarePaginator)
